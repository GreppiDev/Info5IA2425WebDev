...
// Defaults allineati a `.devcontainer/mariadb-entrypoint.sh` e al README.
// Nota: in questo devcontainer MariaDB gira nello *stesso* container.
// Default didattici allineati a `.env.example` e a `.devcontainer/mariadb-entrypoint.sh`.
var host = builder.Configuration["MARIADB_HOST"] ?? "127.0.0.1";
var port = builder.Configuration["MARIADB_PORT"] ?? "3306";
var database = builder.Configuration["MARIADB_DATABASE"] ?? "pizzadb";
var user = builder.Configuration["MARIADB_USER"] ?? "devuser";
var password = builder.Configuration["MARIADB_PASSWORD"] ?? "devpass";
